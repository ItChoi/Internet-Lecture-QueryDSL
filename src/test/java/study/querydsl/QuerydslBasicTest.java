...
public class QuerydslBasicTest {
    ...
    @BeforeEach
    public void before() {
        queryFactory = new JPAQueryFactory(em); // 필드 레벨로 가져가도 된다. 동시성 문제 고민 안해도 된다. 다 해결이 된다.
    ...
    public void startQuerydsl() {
        // JPAQueryFactory queryFactory = new JPAQueryFactory(em); // 필드로 가져가자
    ...
    public void aggregation() {
        // Tuple -> QueryDsl이 제공하는 Tuple
        // 실무에서 Tuple은 많이 쓰지 않고, 실무에서는 DTO로 뽑아오는 방법으로 많이 쓴다.
    ...
    public void theta_join() {
        ...
        // member와 team을 다 가져오고, where에서 거른다.
        // DB가 최적화를 하긴 한다.
        List<Member> result = queryFactory
                .select(member)
                .from(member, team)
    ...
    /**
     * 에) 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인,
     * 회원은 모두 조회
    ...
    public void subQuery() {
        // 바깥에 member랑 겹치면 안된다 (서브 쿼리 사용 시)
    ...
    // 조립하려고 할 땐 BooleanExpression 타입 사용
    ...
    public void bulkUpdate() {
        // member1 = 10, member2 = 20, member1 = 30, member2 = 40
